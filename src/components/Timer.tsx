...
const Timer = ({ onEntryCreated }: TimerProps) => {
  ...
  // ── Screenshot — captures the Cadence app window via html2canvas (no permission needed) ──
  // Note: browsers do not allow silent full-desktop capture without a native extension.
  // This captures the full Cadence app page which is what the browser can access.
  const doScreenshot = async () => {
    ...
    try {
      ...
      // Capture full page — scroll to top first so we get the full view
      const scrollY = window.scrollY;
      window.scrollTo(0, 0);
      const canvas = await (window as any).html2canvas(document.documentElement, {
        scale: 0.6,
        useCORS: true,
        logging: false,
      ...
      if (error) throw error;
      // Silent — no toast so user isn't aware
    } catch (err: any) {
