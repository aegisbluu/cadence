...
interface TimerProps { projectId?: string; onEntryCreated?: () => void; }
...
const Timer = ({ projectId, onEntryCreated }: TimerProps) => {
  ...
  const { data: attendance, refetch: refetchAtt } = useQuery({
    ...
    queryFn: async () => { const { data } = await supabase.from("attendance").select("time_in_at").eq("user_id",user!.id).maybeSingle(); return data; },
    enabled: !!user,
  });
  ...
  useEffect(() => {
    ...
    } else { if (timeInRef.current) clearInterval(timeInRef.current); setTimeInElapsed(0); }
  ...
  const { data: ssInterval=600 } = useQuery({
    ...
    queryFn: async () => { const { data } = await supabase.from("profiles").select("screenshot_interval").eq("user_id",user!.id).single(); return data?.screenshot_interval??600; },
    enabled: !!user,
  });
  ...
  const { data: tasks=[] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from("tasks").select("*, projects(name)").eq("is_completed",false).order("created_at",{ascending:false});
  ...
  const takeScreenshot = useCallback(async () => {
    try {
      ...
      if (ctx) { ctx.fillStyle="#1a1a2e"; ctx.fillRect(0,0,c.width,c.height); ctx.fillStyle="#fff"; ctx.font="20px Inter"; ctx.fillText(`Screenshot at ${new Date().toLocaleTimeString()}`,20,40); ctx.fillText(`Timer: ${fmt(elapsed)}`,20,70); }
      toast({ title:"Screenshot captured" });
    } catch { toast({ title:"Screenshot failed", variant:"destructive" }); }
  },[elapsed,toast]);

  useEffect(() => {
    if (isRunning && mode==="work" && ssInterval>0) { ssRef.current=setInterval(takeScreenshot,ssInterval*1000); }
  ...
  const handleStart = async () => {
    ...
    if (user) await supabase.from("active_timers").upsert({ user_id:user.id, started_at:new Date().toISOString(), task_id:mode==="work"?(activeTaskId||null):null, project_id:projectId||null, mode, is_break:mode==="break", break_started_at:mode==="break"?new Date().toISOString():null },{ onConflict:"user_id" });
  };
  ...
  const handleStop = async () => {
    ...
    const { data: entry } = await supabase.from("time_entries").insert({
      user_id:user.id, task_id:mode==="work"?(activeTaskId||null):null, project_id:projectId||null,
      start_time:startedAt.toISOString(), end_time:new Date().toISOString(), duration_seconds:elapsed,
      screenshot_interval:ssInterval, description:mode==="break"?"Break time":null,
    }).select().single();
    if (mode==="work") { toast({ title:"Time entry saved!" }); qc.invalidateQueries({ queryKey:["task_durations"] }); }
    else { toast({ title:"Break recorded", description:`Break lasted ${fmt(elapsed)}` }); }
  ...
  const timeOutMutation = useMutation({
    mutationFn: async () => {
      ...
      await supabase.from("time_entries").insert({ user_id:user.id, task_id:null, project_id:projectId||null, start_time:timeInStamp.toISOString(), end_time:end.toISOString(), duration_seconds:dur, description:"Attendance: Time In / Time Out" });
      // Update DTR log with time out
  ...
  return (
    <div className="glass-card p-6 space-y-5">
          ...
          {taskOpen && (
            <div className="absolute top-full left-0 right-0 z-30 mt-1 bg-card border border-border rounded-lg shadow-lg overflow-hidden">
              <div className="max-h-56 overflow-y-auto">
                ...
                {tasks.length===0 && <p className="text-xs text-muted-foreground text-center py-4">No tasks — ask admin to add them</p>}
              </div>
            </div>
          )}
        </div>
      ...
      {isRunning && activeTask && <p className="text-xs text-primary text-center">Tracking: <span className="font-medium">{activeTask.name}</span></p>}
        ...
        <div>
          ...
          <div className="space-y-1.5 max-h-[320px] overflow-y-auto">
            {workEntries.map((e:any) => {
              ...
              return (
                <div key={e.id} className={`rounded-lg border px-3 py-2 ${isBreakEntry?"border-warning/40 bg-warning/5":"border-border/50 bg-secondary/50"}`}>
                  {isEditing ? (
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-1">
                        <Button size="sm" className="h-6 text-xs gradient-primary px-2 gap-1" onClick={()=>updateEntry.mutate({ id:e.id, duration_seconds:parseDur(editDur), name:editName, category:editCat, task_id:e.task_id||undefined })}>
                    ...
                    <div>
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5">
                            {isBreakEntry && <Coffee className="h-3 w-3 text-warning flex-shrink-0" />}
                            <p className={`text-xs font-medium truncate ${isBreakEntry?"text-warning":"text-foreground"}`}>{taskName}</p>
                          </div>
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          {!isBreakEntry && (
                            <>
                              ...
                              <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-foreground"
                                onClick={()=>{ setEditingId(e.id); setEditDur(fmtHM(e.duration_seconds||0)); setEditCat(catVal); setEditName(taskName); }}>
                      ...
                      {expandedEntry===e.id && (
                        <div className="mt-2 pl-3 border-l-2 border-primary/30 space-y-1.5">
                          {entryNotes.map((n:any) => (
                            <p key={n.id} className="text-xs text-muted-foreground">· {n.note}</p>
                          ))}
                          <div className="flex gap-1.5 mt-1">
                            <Input value={noteText} onChange={ev=>setNoteText(ev.target.value)} onKeyDown={ev=>{ if (ev.key==="Enter" && noteText.trim() && e.task_id) { addNote.mutate({ taskId:e.task_id, entryId:e.id, note:noteText.trim() }); }}} placeholder="Add note (ticket, meeting…)" className="bg-card border-border text-xs h-6 flex-1" />
                            <Button size="sm" className="h-6 text-xs px-2 gradient-primary" onClick={()=>{ if (noteText.trim() && e.task_id) addNote.mutate({ taskId:e.task_id, entryId:e.id, note:noteText.trim() }); }}>
                              <Plus className="h-3 w-3" />
                            </Button>
                          </div>
                        </div>
